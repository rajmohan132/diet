         ...
         <div class="card">
            <div class="card-header d-flex justify-content-between pb-0 border-0">
               <div class="header-title ">
                  <h4 class="card-title">Assigned Order</h4>
               </div>
            </div>
            <br>
            <div class="row" style="padding-left: 29px;">
            <div class="col-4">
            <input type="date" id="date_from_assigned" class="form-control" placeholder="First name">

            </div>

            <div class="col-4">
            <input type="date" id="date_to_assigned"class="form-control" placeholder="First name">

            </div>
            
          
            <div class="col-2">
            <button onclick="date_filter_assigned()" class="btn btn-primary rounded">Search</button>

            </div>

            
</div>
            <div id="filter_assigned"></div>
            <div class="card-body">
               <div class="table-responsive">
                  <table id="datatable" class="table table-striped nofilterassigned" data-toggle="data-table">
                     <thead>
                        <tr>
...
<script>
function date_filter_assigned(){
   var fromDate = $("#date_from_assigned").val();
   var toDate = $("#date_to_assigned").val();

   $.ajax({
            url:"{{route('filter_order_by_date_assigned')}}",
            method:'POST',
            data:{fromdate:fromDate,todate:toDate, "_token":"{{csrf_token()}}" },
            //dataType:'JSON',
            success:function(res){
               $(".nofilterassigned").hide();
               console.log(res);
               $("#filter_assigned").html(res);
